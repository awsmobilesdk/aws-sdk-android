...
/**
 * JSON request marshaller for DescribeKeyRequest
 */
    ...
    public Request<DescribeKeyRequest> marshall(DescribeKeyRequest describeKeyRequest) {
        if (describeKeyRequest == null) {
            throw new AmazonClientException(
                    "Invalid argument passed to marshall(DescribeKeyRequest)");
        }

        Request<DescribeKeyRequest> request = new DefaultRequest<DescribeKeyRequest>(
                describeKeyRequest, "AmazonLocation");
        request.setHttpMethod(HttpMethodName.GET);

        String uriResourcePath = "/metadata/v0/keys/{KeyName}";
        uriResourcePath = uriResourcePath.replace(
                "{KeyName}",
                (describeKeyRequest.getKeyName() == null) ? "" : StringUtils
                        .fromString(describeKeyRequest.getKeyName()));
